...
func (tq *TagQuery) QueryProductTags() *ProductTagQuery {
	query := (&ProductTagClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withProducts:    tq.withProducts.Clone(),
		withProductTags: tq.withProductTags.Clone(),
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withProductTags; query != nil {
		if err := tq.loadProductTags(ctx, query, nodes,
			func(n *Tag) { n.Edges.ProductTags = []*ProductTag{} },
			func(n *Tag, e *ProductTag) { n.Edges.ProductTags = append(n.Edges.ProductTags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadProductTags(ctx context.Context, query *ProductTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *ProductTag)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(producttag.FieldTagID)
	}
	query.Where(predicate.ProductTag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.ProductTagsColumn), fks...))
	}))
