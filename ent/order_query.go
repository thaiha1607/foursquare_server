...
func (oq *OrderQuery) QueryOrderStatus() *OrderStatusCodeQuery {
	query := (&OrderStatusCodeClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithParentOrder tells the query-builder to eager-load the nodes that are connected to
// the "parent_order" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithParentOrder(opts ...func(*OrderQuery)) *OrderQuery {
	query := (&OrderClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withParentOrder = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withParentOrder; query != nil {
		if err := oq.loadParentOrder(ctx, query, nodes, nil,
			func(n *Order, e *Order) { n.Edges.ParentOrder = e }); err != nil {
			return nil, err
		}
	}
...
func (oq *OrderQuery) loadParentOrder(ctx context.Context, query *OrderQuery, nodes []*Order, init func(*Order), assign func(*Order, *Order)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Order)
	for i := range nodes {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "parent_order_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oq *OrderQuery) loadOrderStatus(ctx context.Context, query *OrderStatusCodeQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderStatusCode)) error {
...
func (oq *OrderQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := oq.ctx.Fields; len(fields) > 0 {
		...
		if oq.withCreator != nil {
			_spec.Node.AddColumnOnce(order.FieldCreatedBy)
		}
		if oq.withParentOrder != nil {
			_spec.Node.AddColumnOnce(order.FieldParentOrderID)
		}
