...
type OrderItemQuery struct {
	...
	withSourceWorkUnit      *WorkUnitInfoQuery
	withDestinationWorkUnit *WorkUnitInfoQuery
...
func (oiq *OrderItemQuery) QueryProduct() *ProductInfoQuery {
	query := (&ProductInfoClient{config: oiq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oiq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oiq *OrderItemQuery) QueryProductColor() *ProductColorQuery {
	query := (&ProductColorClient{config: oiq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oiq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oiq *OrderItemQuery) QuerySourceWorkUnit() *WorkUnitInfoQuery {
	query := (&WorkUnitInfoClient{config: oiq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oiq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oiq *OrderItemQuery) Clone() *OrderItemQuery {
	...
	return &OrderItemQuery{
		...
		withProductColor:        oiq.withProductColor.Clone(),
		withSourceWorkUnit:      oiq.withSourceWorkUnit.Clone(),
		withDestinationWorkUnit: oiq.withDestinationWorkUnit.Clone(),
...
func (oiq *OrderItemQuery) WithProductColor(opts ...func(*ProductColorQuery)) *OrderItemQuery {
	...
	oiq.withProductColor = query
	return oiq
}

// WithSourceWorkUnit tells the query-builder to eager-load the nodes that are connected to
// the "source_work_unit" edge. The optional arguments are used to configure the query builder of the edge.
func (oiq *OrderItemQuery) WithSourceWorkUnit(opts ...func(*WorkUnitInfoQuery)) *OrderItemQuery {
	query := (&WorkUnitInfoClient{config: oiq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oiq.withSourceWorkUnit = query
	return oiq
}

// WithDestinationWorkUnit tells the query-builder to eager-load the nodes that are connected to
// the "destination_work_unit" edge. The optional arguments are used to configure the query builder of the edge.
func (oiq *OrderItemQuery) WithDestinationWorkUnit(opts ...func(*WorkUnitInfoQuery)) *OrderItemQuery {
	query := (&WorkUnitInfoClient{config: oiq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oiq.withDestinationWorkUnit = query
	return oiq
}
...
func (oiq *OrderItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderItem, error) {
	var (
		...
		loadedTypes = [5]bool{
			oiq.withOrder != nil,
			oiq.withProduct != nil,
			oiq.withProductColor != nil,
			oiq.withSourceWorkUnit != nil,
			oiq.withDestinationWorkUnit != nil,
		}
	)
	...
	if query := oiq.withProduct; query != nil {
		if err := oiq.loadProduct(ctx, query, nodes, nil,
			func(n *OrderItem, e *ProductInfo) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	if query := oiq.withProductColor; query != nil {
		if err := oiq.loadProductColor(ctx, query, nodes, nil,
			func(n *OrderItem, e *ProductColor) { n.Edges.ProductColor = e }); err != nil {
			return nil, err
		}
	}
	if query := oiq.withSourceWorkUnit; query != nil {
		if err := oiq.loadSourceWorkUnit(ctx, query, nodes, nil,
			func(n *OrderItem, e *WorkUnitInfo) { n.Edges.SourceWorkUnit = e }); err != nil {
			return nil, err
		}
	}
	if query := oiq.withDestinationWorkUnit; query != nil {
		if err := oiq.loadDestinationWorkUnit(ctx, query, nodes, nil,
			func(n *OrderItem, e *WorkUnitInfo) { n.Edges.DestinationWorkUnit = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oiq *OrderItemQuery) loadProduct(ctx context.Context, query *ProductInfoQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *ProductInfo)) error {
...
func (oiq *OrderItemQuery) loadProductColor(ctx context.Context, query *ProductColorQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *ProductColor)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*OrderItem)
	for i := range nodes {
...
func (oiq *OrderItemQuery) loadSourceWorkUnit(ctx context.Context, query *WorkUnitInfoQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *WorkUnitInfo)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*OrderItem)
	for i := range nodes {
		if nodes[i].SrcUnitID == nil {
			continue
		}
		fk := *nodes[i].SrcUnitID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "src_unit_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oiq *OrderItemQuery) loadDestinationWorkUnit(ctx context.Context, query *WorkUnitInfoQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *WorkUnitInfo)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*OrderItem)
	for i := range nodes {
		if nodes[i].DstUnitID == nil {
			continue
		}
		fk := *nodes[i].DstUnitID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "dst_unit_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oiq *OrderItemQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := oiq.ctx.Fields; len(fields) > 0 {
		...
		if oiq.withProductColor != nil {
			_spec.Node.AddColumnOnce(orderitem.FieldProductColorID)
		}
		if oiq.withSourceWorkUnit != nil {
			_spec.Node.AddColumnOnce(orderitem.FieldSrcUnitID)
		}
		if oiq.withDestinationWorkUnit != nil {
			_spec.Node.AddColumnOnce(orderitem.FieldDstUnitID)
		}
	}
